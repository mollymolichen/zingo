...
<v-toolbar color="pink lighten-5">
    ...
    <div id="right">
        <ul>
            <v-btn v-if="!user" href="#about" class="nav-btn">About</v-btn>
            <router-link v-if="user" :to="{ name: 'EventList', params: { user } }">
                <v-btn class="nav-btn">Events</v-btn>
            </router-link>
            <router-link v-if="user" :to="{ name: 'MatchList', params: { user } }">
            ...
            <router-link v-if="user" :to="{ name: 'Profile', params: { user, updateUser, myProfile: true } }">
